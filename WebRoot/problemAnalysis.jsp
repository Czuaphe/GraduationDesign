...
<script>
    ...
    $(document).ready(function(){
        $.get("http://10.165.215.202:8080/GraduationDesign/problem/show?currentPage=1", function(response) {
            $("#problems").dynamicTables({
                'title' : ['编号', '课题名称', '适合专业', '新题目', '课题类型', '课题来源', '课题性质', '选题方式', '教师姓名','审核状态'],
                'data' : response.data,
                currentPage : response.currentPage,
                totalPage : response.totalPage,
                paginationURL : "http://10.165.215.202:8080/GraduationDesign/problem/show",
                'typeConfig' : [
                    {"edit": false},
                    {"edit" : false},
                    {"edit" : false},
        ...
        $("#submitCommit").click(function(){
            ...
            if(str.length == 0) {
                ...
                $.post("http://10.165.215.202:8080/GraduationDesign/problem/verify", {accepted:false, pro_id:pid, content:str}, function(response){
    ...
    function pass_analysis(target) {
        ...
        $.post("http://10.165.215.202:8080/GraduationDesign/problem/verify", {accepted:true, pro_id:id, content:""}, function(response){
            if(response.status == true) {
                alert("提交成功");
                window.location.reload();
            } else {
    ...
    function show(target){
        var id = $(target).children().eq(0).html();
        $.get("http://10.165.215.202:8080/GraduationDesign/problem/details?pro_id=" + id, function(response){
            if(response.status == true) {
