...
<script>
    ...
    $(document).ready(function(){
        $.get("/GraduationDesign/problem/show?currentPage=1", function(response) {
            $("#problems").dynamicTables({
                'title' : ['编号', '课题名称', '适合专业', '新题目', '课题类型', '课题来源', '课题性质', '选题方式', '教师姓名','审核状态'],
                'data' : response.data,
                currentPage : response.currentPage,
                totalPage : response.totalPage,
                paginationURL : "/GraduationDesign/problem/show",
                'typeConfig' : [
                    {"edit": false},
                    {"edit" : false},
                    {"edit" : false},
        ...
        $("#submitCommit").click(function(){
            ...
            if(str.length == 0) {
                ...
                $.post("/GraduationDesign/problem/verify", {accepted:false, pro_id:pid, content:str}, function(response){
    ...
    function pass_analysis(target) {
        ...
        $.post("/GraduationDesign/problem/verify", {accepted:true, pro_id:id, content:""}, function(response){
            if(response.status == true) {
                alert("提交成功");
                window.location.reload();
            } else {
    ...
    function show(target){
        var id = $(target).children().eq(0).html();
        $.get("/GraduationDesign/problem/details?pro_id=" + id, function(response){
            if(response.status == true) {
